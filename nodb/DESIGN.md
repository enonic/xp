...
**Companion:** [`../CLOUD-ARCHITECTURE.md`](../CLOUD-ARCHITECTURE.md) places NoDB in the
long-term cloud topology and covers tenant-bound XP runtimes, deployment roles, cells,
the control plane, and shared image transformation.
...
| **3** | **Payload storage in NoDB — storage-only** (all structural-query de-search deferred to Phase 8). Store node payloads — the node-data, index-config, and ACL segments — IN NoDB's `payload` table (the split §2 always specified; Phase 1 ran hybrid with payloads on BlobStore — re-add the FK Gate C dropped), behind a stable, versioned, **XP-independent payload format** NoDB can parse as its own bytes (the foundation Phase 8's server-side derivation needs). Search stays on ES; no command/query changes. Binaries already NoDB-fronted (Phase 2). | Payload round-trip green vs NoDB (three segments, dedup preserved); FK re-enforced; format spec documented; default ES mode unchanged; both-backend itests green. | **DONE 2026-07-21** — branch `nodb-phase3-payloads`, gates 0/A–D. Format verdict: XP's serialization IS the v1 payload format (type tags from the closed 14-entry ValueTypes table; REFERENCE identified by tag alone — #10e resolved by documentation; proven by raw-psql read of live payload bytes). Write path: symmetric-B — segment persistence in the SPI store for BOTH backends; nodb = ONE transactional WriteBatch/save (bench: 2.7× vs per-op); ES relocation byte-identical (full default suites unchanged). FK re-added + enforced (#10b resolved). Boot smoke: FK NOT-EXISTS 0/0/0, dedup observed live (48 versions → 2+7 distinct icfg/ACL hashes), file BlobStore 0 bytes, restart-persistent; growth baseline recorded for Phase-5 GC (57 rows/29.5KB). **NoDB is now the complete system of record for structured data in nodb mode.** |
| **4** | OpenSearch index + query-translator port — the **full content/full-text/aggregation search surface** (structural-query de-search is Phase 8, so Phase 4 ports the complete surface); outbox/indexer consuming XP-shipped v1 index documents, with server-side payload derivation explicitly deferred; refresh checkpoint for read-your-writes. Starts with compact prerequisite gates for one protobuf source of truth, repository-scoped version identity, and forward-only/checksummed migrations; operational hardening remains in Phases 5–6. | Prerequisite contract gates green; full core-repo + itest suites green vs NoDB; golden-query corpus diffed against the ES backend; repeatable boot/restart CI smoke and cross-host performance baseline recorded. |
| **5** | Snapshots, vacuum, dump/load verified; retention policies | Ops parity + dump-based migration round-trip test |
| **6** | Production/runtime hardening + control-plane integration: real issuer, membership, break-glass policy, metering/QoS by scope, external ingress, Docker/compose, Helm; multi-XP cache coherence; RPC deadlines/cancellation; bounded upload concurrency/temp disk; real-AWS STS prefix isolation; shared-cell load/fairness. | Quota/QoS and failure-path tests; two XP runtimes for one tenant observe writes coherently; AWS cross-tenant prefix access denied; issuance-to-audit attribution and shared-cell fairness verified end-to-end |
...
10. **Pickups from Phase 1** (all deliberate, recorded in commits/BUILD-PHASE-1),
    ...
    (b) ~~node_version's payload FK dropped~~ **RESOLVED (Phase 3)**: FK re-added and
    enforced (23503 → FAILED_PRECONDITION contract); server-derived `_references` that
    the FK'd payloads enable is **Phase 8**;
    ...
    equivalent (**Phase 6**). New from the payload design discussion: (e) ~~payload format spec~~ **RESOLVED
    (Phase 3 Gate 0)**: XP's existing serialization IS the v1 format — self-describing
    type tags from the closed 14-entry ValueTypes table, REFERENCE identified by tag
    alone; spec documented in BUILD-PHASE-3.md, proven by raw-psql reads of live rows.
