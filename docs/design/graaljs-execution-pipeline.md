...
| A bean parameter typed `List`/`Map`/`Set`/`Iterable` receiving a script value | a detached snapshot | a live view onto the guest object: every read re-enters the script context, so a retained one throws `Multi threaded access requested` once another thread is inside that context, and `The Context is already closed` after the app stops | consume it before returning, or declare `ScriptValue` and take `getList()`/`getMap()`. The platform's defensive copies cover the common case — but only the spine, so a `List<String>` is fully detached while a `List<Map<String, Object>>` keeps guest-backed elements; those need the deep conversion `ScriptValue` performs |
