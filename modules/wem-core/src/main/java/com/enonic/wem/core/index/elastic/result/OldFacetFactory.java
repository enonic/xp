...
public class OldFacetFactory
{
    ...
    public static Facets create( final SearchResponse searchResponse )
    {
        ...
        for ( String facetName : facetsMap.keySet() )
        {
            ...
            {
                facets.addFacet( OldTermFacetFactory.create( (TermsFacet) facet ) );
            }

            else if ( facet instanceof DateHistogramFacet )
            {
                facets.addFacet( OldDateHistogramFacetFactory.create( facetName, (DateHistogramFacet) facet ) );
            }

            else if ( facet instanceof RangeFacet )
            {
                facets.addFacet( OldRangeFacetFactory.create( facetName, (RangeFacet) facet ) );
            }
            else if ( facet instanceof QueryFacet )
            {
                facets.addFacet( OldQueryFacetFactory.create( facetName, (QueryFacet) facet ) );
            }
            /*
