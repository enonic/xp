...
class OrderbyValueResolver
{
    ...
    {
        return longToPrefixCoded( IndexNumericUtils.doubleToSortableLong( val ) );
    }

    private static String floatToPrefixCoded( float val )
    {
        return intToPrefixCoded( IndexNumericUtils.floatToSortableInt( val ) );
    }
    ...
    {
        final char[] buffer = new char[IndexNumericUtils.BUF_SIZE_LONG];
    ...
    private static int longToPrefixCoded( final long val, final int shift, final char[] buffer )
    {
        ...
        buffer[0] = (char) ( IndexNumericUtils.SHIFT_START_LONG + shift );
    ...
    {
        final char[] buffer = new char[IndexNumericUtils.BUF_SIZE_INT];
    ...
    private static int intToPrefixCoded( final int val, final int shift, final char[] buffer )
    {
        ...
        buffer[0] = (char) ( IndexNumericUtils.SHIFT_START_INT + shift );
