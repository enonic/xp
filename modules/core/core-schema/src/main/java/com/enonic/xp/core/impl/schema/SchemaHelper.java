...
public final class SchemaHelper
{
    public static Icon loadIcon( final Class clz, final String metaInfFolderName, final String name )
    {
        ...
        final Optional<Icon> pngIcon = Optional.ofNullable( doLoadIcon( clz, "image/png", filePath + ".png" ) );

        if ( pngIcon.isPresent() )
        {
            return pngIcon.get();
        }
        else
        {
            return doLoadIcon( clz, "image/svg+xml", filePath + ".svg" );
        }
    }
