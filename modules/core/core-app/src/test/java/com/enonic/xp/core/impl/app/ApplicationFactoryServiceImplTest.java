    ...
    extends BundleBasedTest
{
    ...
    @Test
    void bundleApplicationWithNamespace_noDefaultCmsYaml()
        throws Exception
    {
        final BundleContext bundleContext = getBundleContext();
        when( nodeService.findByQuery( any( NodeQuery.class ) ) ).thenReturn( FindNodesByQueryResult.create().build() );
        ...
        final NodePath appPath = new NodePath( NamespaceConstants.NAMESPACE_APP_ROOT_PARENT, NodeName.from( "app1" ) );
    ...
    void findNamespaceApplicationResolver()
    {
        ...
        when( nodeService.nodeExists(
            new NodePath( NamespaceConstants.NAMESPACE_APP_ROOT_PARENT, NodeName.from( applicationKey.getName() ) ) ) ).thenReturn( true );
        ...
        final Optional<ApplicationUrlResolver> resolver = service.findResolver( applicationKey, null );
        assertThat( resolver ).isNotEmpty();
        assertThat( resolver.get() ).isInstanceOf( NodeResourceApplicationUrlResolver.class );
    }
}
