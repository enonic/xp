...
public class MacroParser
{
    ...
    private final static Pattern VALID_MACRO_PATTERN = Pattern.compile( "^\\[\\w*(\\s(\\w*=\\w*))*\\s?(/|\\].*\\[/\\w*)\\]$" );

    private final static Pattern MACRO_WITHOUT_BODY_PATTERN = Pattern.compile( "^\\[(\\w*)((\\s\\w*=\\w*)*)\\s?/\\]$" );

    private final static Pattern MACRO_WITH_BODY_PATTERN = Pattern.compile( "^\\[(\\w*)((\\s\\w*=\\w*)*)\\s?\\](.*)\\[/\\w*\\]$" );
    ...
    private Macro.Builder populateCommonData( final Matcher matcher ) {
        ...
        final Iterable<String> attributesList = Splitter.on( CharMatcher.WHITESPACE ).trimResults().omitEmptyStrings().split( attributesString );
