...
import static com.enonic.xp.core.impl.content.index.processor.PageConfigProcessor.PAGE_COMPONENT_PATH;
...
public class PageConfigProcessorTest
{
        ...
        throws Exception
    {
        ...
        assertEquals( 3, result.getPathIndexConfigs().size() );
    }
        ...
        throws Exception
    {
        final PatternIndexConfigDocument result = processForm( Form.create().build() );
        assertTrue( result.getPathIndexConfigs().contains(
            PathIndexConfig.create().path( PropertyPath.from( PAGE_COMPONENT_PATH, "descriptor" ) ).indexConfig(
        ...
        throws Exception
    {
        ...
        assertTrue( result.getPathIndexConfigs().contains( PathIndexConfig.create().path(
            PropertyPath.from( PAGE_COMPONENT_PATH, CONFIG, descriptorKey.getApplicationKey().toString(), ALL_PATTERN ) ).indexConfig(
            IndexConfig.BY_TYPE ).build() ) );
        assertEquals( "htmlStripper", result.getConfigForPath(
            PropertyPath.from( PAGE_COMPONENT_PATH, CONFIG, descriptorKey.getApplicationKey().toString(),
