...
public final class OccurrenceValidator
{
    ...
    private static final String OPTION_SET_SELECTION_POSTFIX = "_selection";
    ...
    private void validateFormOptionSet( final FormOptionSet formOptionSet, final List<PropertySet> parentDataSets )
    {
        ...
        {
            final List<Property> selectedItems =
                optionSetOccurrencePropertySet.getProperties( formOptionSet.getName() + OPTION_SET_SELECTION_POSTFIX );

            validateOptionSetSelection( formOptionSet, selectedItems );
    ...
    {
        return ( option.isDefaultOption() && selectedItems == null ) ||
            selectedItems.stream().anyMatch( elem -> elem.getString().equals( option.getName() ) );
    }

    private void validateOptionSetSelection( final FormOptionSet formOptionSet, final List<Property> selectedItems )
    {
        if ( selectedItems != null )
        {
            int numberOfSelectedOptions = selectedItems.size();
