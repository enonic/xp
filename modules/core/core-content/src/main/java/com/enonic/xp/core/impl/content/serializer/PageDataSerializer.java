    ...
    extends AbstractDataSetSerializer<Page>
{
    ...
    {
        final RegionDescriptors regionDescriptors = getRegionDescriptors( descriptorKey, componentsAsData );
    ...
    private RegionDescriptors getRegionDescriptors( final DescriptorKey descriptorKey, final List<PropertySet> componentsAsData )
    {
        final PageDescriptor pageDescriptor = pageDescriptorService.getByKey( descriptorKey );
        final boolean isAutoGeneratedDescriptor = pageDescriptor.getModifiedTime() == null;

        if ( isAutoGeneratedDescriptor )
        {
            return componentDataSerializerProvider.getRegionDataSerializer().getRegionDescriptorsAtLevel( 1, componentsAsData );
        }

        return pageDescriptor.getRegions();
    }
