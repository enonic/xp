...
class AccessControlListIndexDocumentFactoryTest
{
    ...
    void single_user()
    {
        ...
        final AccessControlList acl = AccessControlList.create()
            .add( AccessControlEntry.create().allow( Permission.READ ).principal( PrincipalKey.from( "user:myidprovider:rmy" ) ).build() )
            .build();

        final var aclStoreDocumentItems = AccessControlListStoreDocumentFactory.create( acl );
    ...
    void multiple_user()
    {
        final AccessControlList acl = AccessControlList.create()
            .add( AccessControlEntry.create().allow( Permission.READ ).principal( PrincipalKey.from( "user:myidprovider:rmy" ) ).build() )
            .add( AccessControlEntry.create().allow( Permission.READ ).principal( PrincipalKey.from( "user:myidprovider:tsi" ) ).build() )
            .add( AccessControlEntry.create()
                      .allow( Permission.READ )
                      .allow( Permission.DELETE )
                      .principal( PrincipalKey.from( "user:myidprovider:aro" ) )
                      .build() )
            .add( AccessControlEntry.create().allow( Permission.DELETE ).principal( PrincipalKey.from( "user:myidprovider:srs" ) ).build() )
            .build();
