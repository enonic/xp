...
public abstract class BaseSchema<T extends BaseSchemaName>
{
     ...
     * constant for reproducibility. Only a schema stored in a repository node, as a persisted or dynamic
     * application schema is, carries a genuine time, and a caller cannot tell the two apart - so the
