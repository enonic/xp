...
public final class ContentMappingConstraint
{
    ...
    public boolean matches( final Content content )
    {
        ...
        else if ( this.id.startsWith( MIXINS_PROPERTY_PREFIX ) )
        {
            ...
            final PropertyTree xData = getMixins( content.getMixins(), appPrefix, mixinName );
            if ( xData == null )
            {
                return false;
            }
    ...
    private PropertyTree getMixins( final Mixins mixins, final String appPrefix, final String name )
    {
        if ( mixins == null )
        {
            return null;
