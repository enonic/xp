...
public final class UpdateCmsHandler
    implements ScriptBean
{
    ...
    public Object execute()
    {
        final UpdateCmsParams params =
            UpdateCmsParams.create().key( ApplicationKey.from( application ) ).resource( resource ).build();

        final SchemaResult<CmsDescriptor> result = schemaServiceSupplier.get().updateCms( params );
