    ...
    implements ScriptBean
{
    ...
    public PhrasesMapper execute()
    {
        final UpdateDynamicPhrasesParams params =
            UpdateDynamicPhrasesParams.create().key( ApplicationKey.from( application ) ).name( name ).resource( resource ).build();
        return new PhrasesMapper( dynamicSchemaServiceSupplier.get().updatePhrases( params ) );
    }
