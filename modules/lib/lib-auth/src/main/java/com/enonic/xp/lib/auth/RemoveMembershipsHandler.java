...
public final class RemoveMembershipsHandler
        ...
        if ( authInfo.isAuthenticated() )
        {
            if ( this.membershipsList != null && this.principalKey != null )
            {
                for ( final String key : this.membershipsList )
                {
                    ...
                    this.securityService.get().removeRelationship(
                        PrincipalRelationship.from( principalKeyToAdd ).to( this.principalKey ) );
                }
