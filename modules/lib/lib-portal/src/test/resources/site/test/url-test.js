...
exports.applicationUrlTest = function () {
    var result = portal.applicationUrl({
        application: 'myapp',
        path: '/subpath',
        params: {
            a: 1,
    ...
    // NOTE: This is not the actual url. Only a mock representation.
    assert.assertEquals('ApplicationUrlParams{type=server, params={a=[1], b=[1, 2]}, path=/subpath, application=myapp}', result);
    return true;
};
