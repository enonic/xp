    ...
    implements XmlObject<Property, DataSet>, DataXml
{
    ...
    public void to( final DataSet output )
    {
        ...
        if ( valueType.equals( ValueTypes.DATA ) )
        {
            ...
            for ( Object value : this.values )
            {
                if ( value instanceof RootDataSetValueXml )
                {
                    ...
                    final Property property = Property.newProperty( this.name, Value.newValue( valueType, valueRootDataSet ) );
                    output.add( property );
                    found = true;
                }
            }
        ...
        {
            final Property property = Property.newProperty( this.name, Value.newValue( valueType, this.values.get( 0 ) ) );
            output.add( property );
        }
    }
}
