    ...
    implements ScriptRuntimeFactory, ApplicationInvalidator, ApplicationListener
{
    ...
    private final GraalJsEngineProvider engineProvider;

    @Activate
    public ScriptRuntimeFactoryImpl( @Reference final GraalJsEngineProvider engineProvider,
    ...
    ScriptRuntimeImpl doCreate( final ScriptSettings settings )
    {
        final ScriptExecutorFactory scriptExecutorFactory =
            new ScriptExecutorFactory( engineProvider.getEngine(), applicationService, resourceService, scriptAsyncService, settings );
