...
public class ResolveDependenciesAggregationFactoryTest
{
    ...
    public void resolve_inbound_dependencies()
        throws Exception
    {
        Content content = createContent( "folderRefContent1", new PropertyTree(), ContentTypeName.folder() );

        FindContentByQueryResult findContentByQueryResult = FindContentByQueryResult.create().aggregations( Aggregations.from( BucketAggregation.bucketAggregation( "type" ).
        ...
        DependenciesJson result = factory.create( content.getId() );
        ...
        throws Exception
    {
        ...
        Content folderRefContent1 = createContent( "folderRefContent1", data, ContentTypeName.folder() );

        Content folderRefContent2= createContent( "folderRefContent2", data, ContentTypeName.folder() );

        Content siteRefContent1= createContent( "siteRefContent1", data, ContentTypeName.site() );
        ...
        Content content = createContent( "content", data, ContentTypeName.site() );
        ...
        Mockito.when( contentService.getById( content.getId() ) ).thenReturn(content);
        Mockito.when( contentService.getById( folderRefContent1.getId() ) ).thenReturn(folderRefContent1);
        Mockito.when( contentService.getById( folderRefContent2.getId() ) ).thenReturn(folderRefContent2);
        Mockito.when( contentService.getById( siteRefContent1.getId() ) ).thenReturn(siteRefContent1);
