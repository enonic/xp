    ...
    implements JaxRsComponent
{
    ...
    public DeleteContentResultJson delete( final DeleteContentJson json )
    {
        ...
        final List<ContentId> onlineContentToDelete = new ArrayList<>();
        ...
        for ( final ContentPath contentToDelete : contentsToDeleteList )
        {
            ...
            try
            {
                ...
                contents.forEach( ( content ) -> {
                    ...
                    {
                        if ( json.isDeleteOnline() )
                        {
                            onlineContentToDelete.add( content.getId() );
                        }
                        else
                        {
        ...
        if ( json.isDeleteOnline() )
        {
            final PushContentsResult result = contentService.push( PushContentParams.create().
                    ...
                    contentIds( ContentIds.from( onlineContentToDelete ) ).
                    includeChildren( true ).
                    includeDependencies( true ).
                    build() );

            result.getDeletedContents().forEach( (content) -> jsonResult.addSuccess( content.getId().toString(), content.getDisplayName(), content.getType().getLocalName() ) );
            result.getFailedContents().forEach( (content) -> jsonResult.addFailure( content.getId().toString(), content.getDisplayName(), content.getType().getLocalName(), null ) );
        }
