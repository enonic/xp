    ...
    extends AbstractDataRpcHandler
{
        ...
        throws Exception
    {
        final QualifiedContentTypeNames contentTypeNames =
            QualifiedContentTypeNames.from( context.param( "contentTypes" ).required().asStringArray() );

        final DeleteContentTypes deleteContentTypes = Commands.contentType().delete();
        deleteContentTypes.names( contentTypeNames );
        Integer deletedCount = client.execute( deleteContentTypes );
        context.setResult( new DeleteContentTypeJsonResult( deletedCount ) );
    }
}
