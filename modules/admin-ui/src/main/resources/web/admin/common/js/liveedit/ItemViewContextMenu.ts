module api.liveedit {
    ...
    export class ItemViewContextMenu extends api.dom.DivEl {
        ...
        private oveflowsBottom(y:number, notClicked?: boolean): boolean {
            if (notClicked) {
                return this.overflowsFrameHeight(y);
            }
            else {
                return this.overflowsVisibleHeight(y);
            }
        }

        private overflowsFrameHeight(y:number): boolean {
            return (y + this.getEl().getHeight() + this.arrow.getHeight() + 1) > document.body.scrollHeight;
        }

        private overflowsVisibleHeight(y:number): boolean {
