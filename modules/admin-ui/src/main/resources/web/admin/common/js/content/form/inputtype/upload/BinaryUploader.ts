module api.content.form.inputtype.upload {
    ...
    export class BinaryUploader extends api.form.inputtype.support.BaseInputTypeSingleOccurrence<any,string> {
        ...
        constructor(config: api.content.form.inputtype.ContentInputTypeViewContext<any>) {
            this.inputName = config.input.getName();
            this.contentId = config.contentId.toString();

            super(config, "binary-uploader");
        }
    ...
    api.form.inputtype.InputTypeManager.register(new api.Class("BinaryUploader", BinaryUploader));
}