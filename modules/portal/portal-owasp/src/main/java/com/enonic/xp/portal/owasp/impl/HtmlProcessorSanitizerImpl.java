...
@Component(immediate = true, property = "type=processor")
public final class HtmlProcessorSanitizerImpl
    implements HtmlSanitizer
{
    ...
    @Activate
    public HtmlProcessorSanitizerImpl()
    {
        ...
        return ( elementName, attrs ) -> {
            if ( "img".equals( elementName ) )
            {
                if ( attrs.contains( "src" ) )
                {
                    ...
                    {
                        if ( allowedAttrs.contains( attrs.get( i ) ) )
                        {
