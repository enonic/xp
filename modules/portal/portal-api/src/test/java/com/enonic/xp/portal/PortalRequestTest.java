...
public class PortalRequestTest
{
    ...
    {
        final PortalRequest request = new PortalRequest();
        assertEquals( ContentConstants.BRANCH_ID_DRAFT, request.getBranchId() );

        request.setBranchId( BranchId.from( "another" ) );
        assertEquals( BranchId.from( "another" ), request.getBranchId() );
    }
