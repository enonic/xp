 ...
 *     <li>{@code allowRedirectUri(req)} - redirect policy. When implemented, it owns redirect
...
public class DeviceLoginHandler
{
     ...
     * hook it owns redirect validation entirely - it decides every target itself (loopback included)
     * and returns a {@link PortalResponse} to reject the request, or nothing to allow it. If the id
     * provider does not implement the hook, core falls back to allowing only loopback redirects (any
     * port) - they never leave the device - and rejecting everything else. The deciding id provider is
     * the one addressed in the request URL (it issues the token), so there is no ambiguity about which
     * one decides.
     *
