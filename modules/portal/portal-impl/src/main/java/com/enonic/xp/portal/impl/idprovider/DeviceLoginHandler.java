 ...
 * <p>
 * The only id-provider-specific step - the human approval / consent page - is delegated to a
 * predefined controller hook ({@code approval(req, context)}) the way {@code autoLogin} is invoked:
 * the id provider implements only the branded UI and never routes or reads flow config. Per-vhost
 * flow gating (DEVICE / NATIVE) is enforced by the caller and re-checked here per grant.
 */
@Component(service = DeviceLoginHandler.class)
@NullMarked
public class DeviceLoginHandler
{
    static final String APPROVAL_FUNCTION = "approval";
        ...
        throws IOException
    {
        ...
        return approval( req, idProvider, deviceContext( req, user, status, userCode == null ? null : userCode.toUpperCase() ) );
    }
        ...
        throws IOException
    {
        ...
        {
            return approval( req, idProvider, deviceContext( req, user, "invalid", userCode ) );
        }

        deviceAuthService.resolve( idProvider, deviceCode.get(), approve, approve ? user.getKey() : null );
        return approval( req, idProvider, deviceContext( req, user, approve ? "approved" : "denied", userCode ) );
    }
    ...
    private PortalResponse authorizePrompt( final PortalRequest req, final IdProviderKey idProvider )
        throws IOException
    {
        final PortalResponse validation = validateAuthorize( req );
        if ( validation != null )
        {
            return validation;
        }
        ...
        return approval( req, idProvider, nativeContext( req, user ) );
    }
    ...
    private PortalResponse authorizeSubmit( final PortalRequest req, final IdProviderKey idProvider )
    {
        ...
        final String code = nativeAuthCodeStore.create( idProvider,
                                                        new NativeAuthCodeStore.AuthCode( param( req, "code_challenge" ), redirectUri,
                                                                                          user.getKey().toString(), orEmpty( param( req, "client_id" ) ),
    ...
    @Nullable
    private PortalResponse validateAuthorize( final PortalRequest req )
    {
        final String redirectUri = param( req, "redirect_uri" );
        if ( redirectUri == null || !isAllowedRedirect( redirectUri ) )
        {
    ...
    /**
     * Invokes the id provider's predefined {@code approval} hook to render the human-facing page,
     * passing the approval context as its second argument. Falls back to a built-in page if the
     * id provider does not implement the hook.
     */
    private PortalResponse approval( final PortalRequest req, final IdProviderKey idProvider, final ApprovalContext context )
        throws IOException
    {
        final PortalResponse response = idProviderControllerService.execute( IdProviderControllerExecutionParams.create()
                                                                                 ...
                                                                                 .contextArg( context )
                                                                                 .build() );
        return response != null ? response : fallbackApprovalPage( context );
    }

    private ApprovalContext deviceContext( final PortalRequest req, final User user, final String status, @Nullable final String userCode )
    {
        final Map<String, Object> values = new LinkedHashMap<>();
        values.put( "flow", "device" );
        ...
        putUser( values, user );
        return new ApprovalContext( values );
    }

    private ApprovalContext nativeContext( final PortalRequest req, final User user )
    {
        final Map<String, Object> values = new LinkedHashMap<>();
        values.put( "flow", "native" );
        values.put( "status", "consent" );
        ...
        for ( final String name : new String[]{"redirect_uri", "code_challenge", "code_challenge_method", "scope", "state", "client_id",
            "resource"} )
        {
    ...
    // ---------------------------------------------------------------------------
    // Built-in fallback approval page (the id provider SHOULD override via the hook).
    // ---------------------------------------------------------------------------

    private static PortalResponse fallbackApprovalPage( final ApprovalContext context )
    {
        final Map<String, Object> v = context.values;
        final String status = String.valueOf( v.get( "status" ) );
        final String action = escape( String.valueOf( v.get( "actionUrl" ) ) );
        final String who = escape( String.valueOf( v.getOrDefault( "userDisplayName", "" ) ) );

        final String inner;
        switch ( status )
        {
            case "need_code":
                inner = "<h1>Device sign-in</h1><p>Enter the code shown by your application.</p>" +
                    "<form method=\"get\" action=\"" + action + "\">" +
                    "<input type=\"text\" name=\"user_code\" placeholder=\"XXXX-XXXX\" autofocus>" +
                    "<div><button type=\"submit\">Continue</button></div></form>";
                break;
            case "confirm":
                inner = "<h1>Confirm device sign-in</h1><p>You are signed in as <strong>" + who + "</strong>.</p>" +
                    "<p>A device is requesting access using code <code>" + escape( String.valueOf( v.get( "userCode" ) ) ) + "</code>.</p>" +
                    "<form method=\"post\" action=\"" + action + "\">" + hidden( "user_code", String.valueOf( v.get( "userCode" ) ) ) +
                    "<button type=\"submit\" name=\"approve\" value=\"true\">Approve</button> " +
                    "<button type=\"submit\" name=\"approve\" value=\"false\">Deny</button></form>";
                break;
            case "consent":
                inner = "<h1>Authorize application</h1><p>You are signed in as <strong>" + who + "</strong>.</p>" +
                    "<p>An application is requesting access to sign in as you.</p>" + "<form method=\"post\" action=\"" + action + "\">" +
                    hiddenEchoed( v ) + "<button type=\"submit\" name=\"approve\" value=\"true\">Allow</button> " +
                    "<button type=\"submit\" name=\"approve\" value=\"false\">Deny</button></form>";
                break;
            case "approved":
                inner = "<h1>You're all set</h1><p>The device has been approved. You can return to your application.</p>";
                break;
            case "denied":
                inner = "<h1>Request denied</h1><p>The sign-in request was denied.</p>";
                break;
            case "invalid":
            default:
                inner = "<h1>Device sign-in</h1><p>This code is invalid or has expired.</p>";
                break;
        }
    ...
    private static String hiddenEchoed( final Map<String, Object> v )
    {
            ...
            "resource"} )
        {
            ...
            {
                sb.append( hidden( name, String.valueOf( v.get( name ) ) ) );
            }
        }
        return sb.toString();
    }

    private static String hidden( final String name, final String value )
    {
    ...
    static boolean isAllowedRedirect( final String uri )
    {
        ...
        // which mitigates code interception. Remote http(s) redirects need registration (out of scope).
    ...
    private static PortalResponse html( final String body )
    {
        return PortalResponse.create().status( HttpStatus.OK ).contentType( MediaType.HTML_UTF_8 ).body( body ).header( "Cache-Control", "no-store" ).build();
    }
        ...
        implements MapSerializable
    {
        ...
        private ApprovalContext( final Map<String, Object> values )
        {
            this.values = values;
        }
