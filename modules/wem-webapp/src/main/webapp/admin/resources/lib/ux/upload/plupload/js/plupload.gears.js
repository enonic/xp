            ...
            function o() {
                ...
                function z(D) {
                    ...
                    if (x) {
                        ...
                        C.append(F + I + H + 'Content-Disposition: form-data; name="' + t.settings.file_data_name + '"; filename="' +
                                 q.name + '"' + H + "Content-Type: " + G + H + H);
                        C.append(D);
                        C.append(H + F + I + F + H);
                        E = C.getAsBlob();
                        w = E.length - D.length;
                ...
                h.onreadystatechange = function () {
                    ...
                    if (h.readyState == 4 && t.state !== c.STOPPED) {
                        if (h.status == 200) {
                            ...
                            t.trigger("Error",
                                {code: c.HTTP_ERROR, message: c.translate("HTTP Error."), file: q, chunk: v, chunks: u, status: h.status})
                        }
                    }
                };
                if (v < u) {
