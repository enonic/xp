...
class SystemDumpListenerImplTest
{
    @Test
    void progressAcrossBranches()
    {
        final ProgressReporter progressReporter = mock( ProgressReporter.class );
        final SystemDumpListenerImpl listener = new SystemDumpListenerImpl( progressReporter );

        listener.totalBranches( 2 );
        listener.dumpingBranch( RepositoryId.from( "repo" ), Branch.from( "draft" ), 2 );
        listener.nodeDumped();
        listener.nodeDumped();
        listener.dumpingBranch( RepositoryId.from( "repo" ), Branch.from( "master" ), 4 );
        listener.nodeDumped();
