...
public class IgniteSessionDataStoreTest
{
        ...
        throws Exception
    {
        ...
        verify( cache, Mockito.times( 1 ) ).putAsync( eq( "cpath_vhost_123" ), any( SessionData.class ) );
    }
        ...
        throws Exception
    {
        ...
        when( cache.removeAsync( anyString() ) ).thenReturn( removeFuture );
        final IgniteFuture<Void> putFuture = new IgniteFinishedFutureImpl<>();
        when( cache.putAsync( anyString(), any( SessionData.class ) ) ).thenReturn( putFuture );
