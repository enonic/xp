...
public final class WebResponseSerializer
{
    private final WebRequest webRequest;

    private final WebResponse webResponse;

    public WebResponseSerializer( final WebRequest webRequest, final WebResponse webResponse )
    {
        ...
        throws IOException
    {
        ...
        response.setStatus( webResponse.getStatus().value() );
        response.setContentType( webResponse.getContentType().toString() );

        serializeHeaders( response, webResponse.getHeaders() );
        serializeCookies( response, webResponse.getCookies() );
        serializeBody( response, webResponse.getBody() );
    }

    private void serializeCookies( final HttpServletResponse response, final Collection<Cookie> cookies )
    {
